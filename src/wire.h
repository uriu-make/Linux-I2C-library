...
class wire {
 ...
 public:
  ...
  int WriteReg(unsigned char reg, const void *data, int length = 1, bool resterted = false);

  int Read(void *data, const int length = 1);
  /*"length" does not include the number of bytes in the "reg".*/
  int ReadReg(unsigned char reg, void *data, const int length = 1, bool resterted = false);
